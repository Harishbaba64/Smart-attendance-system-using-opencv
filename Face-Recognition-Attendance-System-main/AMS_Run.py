...
# Add image file
bg = PhotoImage(file = "msec2.png")
  
# Create Canvas
canvas1 = Canvas( window, width = 1600,
                 height = 1600)
...
def manually_fill():
    ...
    def err_screen_for_subject():
        ...
        Label(ec, text='Please enter your subject name!!!', fg='red',
              bg='white', font=('castellar', 16, ' bold ')).pack()
        Button(ec, text='OK', command=ec_delete, fg="black", bg="lawn green", width=9, height=1, activebackground="Red",
               font=('castellar', 15, ' bold ')).place(x=90, y=50)

    def fill_attendance():
        ts = time.time()
        Date = datetime.datetime.fromtimestamp(ts).strftime('%Y_%m_%d')
        castellartamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
        Time = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
        Hour, Minute, Second = castellartamp.split(":")
        ...
        try:
            ...
            conn = pymysql.connect(
                host='127.0.0.1', user='root', password='YES', db='manually_fill_attendance')
            cursor = conn.cursor()
        ...
        if subb == '':
            ...
            def err_screen1():
                ...
                Label(errsc2, text='Please enter Student & Enrollment!!!', fg='black', bg='white',
                      font=('castellar', 16, ' bold ')).pack()
                Button(errsc2, text='OK', command=del_errsc2, fg="black", bg="lawn green", width=9, height=1,
                       activebackground="Red", font=('castellar', 15, ' bold ')).place(x=90, y=50)
            ...
            ENR_ENTRY = tk.Entry(MFW, width=20, validate='key',
                                 bg="white", fg="black", font=('castellar', 23))
            ...
            STUDENT_ENTRY = tk.Entry(
                MFW, width=20, bg="white", fg="black", font=('castellar', 23))
            ...
            def create_csv():
                ...
                csv_name = r'C:\Users\haris\OneDrive\Desktop\Face-Recognition-Attendance-System-main/Manually Attendance/'+DB_table_name+'.csv'
                with open(csv_name, "w") as csv_file:
                    ...
                    Notifi.configure(text=O, bg="Green", fg="white",
                                     width=33, font=('castellar', 19, 'bold'))
                ...
                with open(csv_name, newline="") as file:
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            ...
                            label = tkinter.Label(root, width=18, height=1, fg="black", font=('castellar', 13, ' bold '),
    ...
    SUB = tk.Label(sb, text="Enter Subject : ", width=15, height=2,
                   fg="black", bg="grey80", font=('castellar', 15, ' bold '))
    SUB.place(x=30, y=100)

    global SUB_ENTRY

    SUB_ENTRY = tk.Entry(sb, width=20, bg="white",
                         fg="black", font=('castellar', 23))
    ...
    if l1 == '':
        ...
        try:
            ...
            Notification.configure(
                text=res, bg="SpringGreen3", width=50, font=('castellar', 18, 'bold'))
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    ...
                    Notifica.configure(
                        text=e, bg="red", fg="black", width=33, font=('castellar', 15, 'bold'))
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if (conf < 70):
                            ...
                            global aa
                            global date
                            global castellartamp
                            Subject = tx.get()
                            ts = time.time()
                            date = datetime.datetime.fromtimestamp(
                                ts).strftime('%Y-%m-%d')
                            castellartamp = datetime.datetime.fromtimestamp(
                                ts).strftime('%H:%M:%S')
                ...
                insert_data = "INSERT INTO " + DB_Table_name + \
                    " (ID,ENROLLMENT,NAME,DATE,TIME) VALUES (0, %s, %s, %s,%s)"
                VALUES = (str(Id), str(aa), str(date), str(castellartamp))
                try:
                    cursor.execute(sql)  # for create a table
                ...
                Notifica.configure(text=M, bg="Green", fg="white",
                                   width=33, font=('castellar', 15, 'bold'))
                Notifica.place(x=20, y=250)
                ...
                with open(cs, newline="") as file:
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            # i've added some styling
                            label = tkinter.Label(root, width=10, height=1, fg="black", font=('castellar', 15, ' bold '),
                                                  bg="white", text=row, relief=tkinter.RIDGE)
                            label.grid(row=r, column=c)
                            c += 1
                        r += 1
    ...
    Notifica = tk.Label(windo, text="Attendance filled Successfully", bg="Green", fg="white", width=30,
                        height=2, font=('castellar', 13, 'bold'))

    def Attf():
        import subprocess
        subprocess.Popen(
            r'C:\Users\haris\OneDrive\Desktop\Face-Recognition-Attendance-System-main\Attendance')


    sub = tk.Label(windo, text="Enter Subject : ", width=15, height=2,
                   fg="black", bg="grey", font=('castellar', 13, ' bold '))
    sub.place(x=30, y=100)
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'pragya':
            if password == 'pragya123':
                ...
                file=open (r'C:\Users\haris\OneDrive\Desktop\Face-Recognition-Attendance-System-main\StudentDetails.csv')
                reader = csv.reader(file)
                ...
                Nt.configure(text=valid, bg="red", fg="white",
                             width=38, font=('castellar', 19, 'bold'))
    ...
    Login = tk.Button(win, text="LogIn", fg="black", bg="SkyBlue1", width=20,
                      height=2,
                      activebackground="Red", command=log_in, font=('castellar', 15, ' bold '))
...
def trainimg():
    ...
    try:
        ...
        Notification.configure(text=l, bg="SpringGreen3",
                               width=50, font=('castellar', 18, 'bold'))
...
message = tk.Label(window, text="SMART ATTENDANCE SYSTEM", bg="black", fg="white", width=43,
                   height=3, font=('castellar', 30, ' bold '))

message.place(x=80, y=20)

Notification = tk.Label(window, text="All things good", bg="Green", fg="white", width=15,
                        height=3, font=('castellar', 17))

lbl = tk.Label(window, text="Enter Enrollment : ", width=20, height=2,
               fg="white", bg="black", font=('castellar', 15, 'bold'))
...
txt = tk.Entry(window, validate="key", width=20, bg="white",
               fg="black", font=('castellar', 25))
txt['validatecommand'] = (txt.register(testVal), '%P', '%d')
txt.place(x=550, y=210)

lbl2 = tk.Label(window, text="Enter Name : ", width=20, fg="white",
                bg="black", height=2, font=('castellar', 15, ' bold '))
...
clearButton = tk.Button(window, text="Clear", command=clear, fg="white", bg="black",
                        width=10, height=1, activebackground="white", font=('castellar', 15, ' bold '))
...
takeImg = tk.Button(window, text="Take Images", command=take_img, fg="black", bg="SkyBlue1",
                    width=15, height=3, activebackground="white", font=('castellar', 15, ' bold '))
...
FA = tk.Button(window, text="fill Attendance", fg="black", command=subjectchoose,
               bg="SkyBlue1", width=15, height=3, activebackground="white", font=('castellar', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manual Attendance", command=manually_fill, fg="black",
                       bg="SkyBlue1", width=18, height=3, activebackground="white", font=('castellar', 15, ' bold '))
